...
/**
 * Provides a form for reverting a Vendor record revision.
 *
 * @ingroup asset_management
 */
class VendorRecordRevisionRevertForm extends ConfirmFormBase {
  ...
  public function getFormId() {
    return 'vendor_record_revision_revert_confirm';
  }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $this->logger('content')->notice('Vendor record: reverted %title revision %revision.', ['%title' => $this->revision->label(), '%revision' => $this->revision->getRevisionId()]);
    $this->messenger()->addMessage(t('Vendor record %title has been reverted to the revision from %revision-date.', ['%title' => $this->revision->label(), '%revision-date' => $this->dateFormatter->format($original_revision_timestamp)]));
   ...
   *
   * @param \Drupal\asset_management\Entity\VendorRecordInterface $revision
   ...
   */
  protected function prepareRevertedRevision(VendorRecordInterface $revision, FormStateInterface $form_state) {
