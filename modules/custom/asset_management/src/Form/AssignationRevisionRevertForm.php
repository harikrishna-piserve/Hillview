...
/**
 * Provides a form for reverting a Assignation revision.
 *
 * @ingroup asset_management
 */
class AssignationRevisionRevertForm extends ConfirmFormBase {
  ...
  public function getFormId() {
    return 'assignation_revision_revert_confirm';
  }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $this->logger('content')->notice('Assignation: reverted %title revision %revision.', ['%title' => $this->revision->label(), '%revision' => $this->revision->getRevisionId()]);
    $this->messenger()->addMessage(t('Assignation %title has been reverted to the revision from %revision-date.', ['%title' => $this->revision->label(), '%revision-date' => $this->dateFormatter->format($original_revision_timestamp)]));
   ...
   *
   * @param \Drupal\asset_management\Entity\AssignationInterface $revision
   ...
   */
  protected function prepareRevertedRevision(AssignationInterface $revision, FormStateInterface $form_state) {
